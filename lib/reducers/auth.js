...
const INITIAL_STATE = {
  token: _appData.default.token || null,
  user: _appData.default.user || null,
  ...
  switch (type) {
    ...
    case AUTH_ACCEPT_CONNECTION_SUCCESS:
      return { ...state,
        busy: false,
        user: { ...state.user,
          consults: [action.consult, ...(state.user.consults || [])]
        },
        connectionProposals: (state.connectionProposals || []).filter(connection => connection._id !== action.connectionId)
      };
    ...
    case AUTH_REMOVE_CONNECTION_SUCCESS:
      return { ...state,
        busy: false,
        user: { ...state.user,
          consults: (state.user.consults || []).filter(consult => consult._id !== action.consultId)
        }
      };
