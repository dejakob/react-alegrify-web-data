...
async function login(action) {
    ...
    const { token } = await ApiFetch.postAnonymous('/api/auth/login', {
        userName,
        password
    });
...
async function signup(action) {
    const { userName, email, password, notifyMe } = action;
    ...
    const { token } = await ApiFetch.postAnonymous('/api/auth/register', {
        userName,
        email,
        password,
        notify_me: notifyMe
...
async function acceptConnection(action) {
    ...
    const { user } = await ApiFetch.post('/api/connect/approve', {
...
async function rejectConnection(action) {
    ...
    await ApiFetch.post('/api/connect/decline', {
        connectionProposalId: connectionId
    });
...
async function removeConnection(action) {
    const { consultId } = action;

    await ApiFetch.remove('/api/connect/from/consult', {
        userId: consultId
    });
